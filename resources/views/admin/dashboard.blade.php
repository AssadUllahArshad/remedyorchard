      ...
      @if($categoryBreakdown->isNotEmpty())
        ...
        <div class="d-flex justify-content-between align-items-center mb-2">
          <span style="font-size:0.88rem; color:var(--text-on-light-dim);">
            <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#2b5fa3;margin-right:0.5rem;"></span>
            {{ $category->name }}
          </span>
          <span style="font-size:0.88rem; font-weight:700;">{{ $category->articles_count }}</span>
        </div>
        @endforeach
      @else
        <p class="table-row-sub">No categories have articles yet.</p>
      @endif
    </div>
  </div>
</div>
